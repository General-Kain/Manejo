...
";
    ...
    private static void NormalizeAllLineEndings() {
        ...
        string relativePath = RCCP_AssetUtilities.BasePath;
        if (string.IsNullOrEmpty(relativePath)) return;

        // Convert to system path
        // BasePath starts with "Assets/", so we strip it to append to Application.dataPath
        string systemPath = Application.dataPath + relativePath.Substring("Assets".Length);

        // Get every C# file under RCCP folder
        var allCS = Directory.GetFiles(systemPath, "*.cs", SearchOption.AllDirectories);
